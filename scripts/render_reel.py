...
def make_overlay(scene, brand, width, height, out_path: Path):
    ...
    d.multiline_text(
        (width // 2, 1285), headline, font=main_font, anchor="mm", align="center",
        fill=(255, 255, 255, 255), spacing=8, stroke_width=3, stroke_fill=(0, 0, 0, 220),
    )
...
def render_scene(image_path: Path, overlay_path: Path, out_path: Path, duration: float, width: int, height: int, fps: int):
    ...
    vf = (
        ...
        f"format=yuv420p[bg];[bg][1:v]overlay=0:0:format=auto"
    )
    cmd = [
        "ffmpeg", "-y", "-loop", "1", "-i", str(image_path), "-loop", "1", "-i", str(overlay_path),
        "-filter_complex", vf, "-t", f"{duration}", "-r", str(fps), "-an", "-c:v", "libx264",
